...
class JsonRPCMethod(object):
    def __init__(self, ws, method, timeout=30):
        self.ws, self.method, self.timeout = ws, method, timeout
    ...
    def __call__(self, *args, **kwargs):
        ...
        jsonresult = json.loads(self.ws.recv().decode("utf-8"))
        print json.dumps(jsonresult, indent=2, ensure_ascii=False)
...
class JsonRPCClient(object):
    def __init__(self, ws, timeout=30, method_class=JsonRPCMethod):
        self.ws = ws
        self.timeout = timeout
        self.method_class = method_class

    def __getattr__(self, method):
        return self.method_class(ws=self.ws, method=method, timeout=self.timeout)
...
class AutomatorServer(object):
    ...
    def jsonrpc_wrap(self, timeout):
        return JsonRPCClient(ws=self.ws, timeout=timeout, method_class=JsonRPCMethod)


class Selector(dict):  # 就是一个字典{'':'','',''}
    __fields = {
    ...
    __mask, __childOrSibling, __childOrSiblingSelector = "mask", "childOrSibling", "childOrSiblingSelector"  # 子集和同级
...
class AutomatorDevice(object):
    ...
    def __init__(self, packageName=None, url=u"ws://10.12.42.142:5000"):
        self.server = AutomatorServer(websocket.create_connection(url))
    ...
    def openApp(self):
        return self.server.jsonrpc.openApp(self.packageName)

    def clearData(self):
        return self.server.jsonrpc.clearData(self.packageName)
    ...
    def dump(self, filename='hierarchy.xml', compressed=True, pretty=True):
    ...
    def screenshot(self, filename="screenshot.png", scale=1.0, quality=100):
        device_file = self.server.jsonrpc.takeScreenshot(filename, scale, quality)
        if not device_file:
            return None
        return filename
...
if __name__ == '__main__':
    ...
    print device.info
    print device.screen.on()
    print device.drag(400, 1500, 400, 800, 20)
    time.sleep(2)
    print device.swipePoints([(84, 960), (450, 960), (896, 960), (896, 1326), (896, 1693)], 30)
